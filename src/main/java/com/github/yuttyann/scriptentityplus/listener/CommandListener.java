...
public class CommandListener implements Listener {
    ...
    private static final CommandLogFilter FILTER = new CommandLogFilter();

    static {
        if (ROOT_LOGGER instanceof Logger) {
            ((Logger) ROOT_LOGGER).addFilter(FILTER);
        }
    }

    public static void removeFilter() {
        if (ROOT_LOGGER instanceof Logger) {
            ((Logger) ROOT_LOGGER).get().removeFilter(FILTER);
        }
    }
